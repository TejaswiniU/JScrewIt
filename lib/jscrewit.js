...
(function () {
    ...
    function noProto(obj) {
        ...
        _Object_keys(obj).forEach(function (name) {
    ...
    function compareClustersByQuality(cluster1, cluster2) {
        var diff = cluster1.saving - cluster2.saving ||
    ...
    function conclude() {
        ...
        var _a = this, maxLength = _a.maxLength, startLinks = _a.startLinks;
        var bestClusters = [];
        var cluster;
    ...
    function unlinkClusters(startLinks, maxLength, cluster) {
        ...
        for (var length_1 = 1; length_1 < maxLength;) {
            startLink = startLinks[start - length_1++];
            if (startLink) {
                unlinkClustersFromLength(startLink, length_1);
                startLink.length = length_1;
            }
        }
    }
    function unlinkClustersFromLength(startLink, fromLength) {
        for (var length_2 = startLink.length; length_2-- > fromLength;) {
            var cluster = startLink[length_2];
            if (cluster)
    ...
    var Level;
    (function (Level) {
        Level[Level["UNDEFINED"] = -2] = "UNDEFINED";
        Level[Level["NUMERIC"] = -1] = "NUMERIC";
        Level[Level["OBJECT"] = 0] = "OBJECT";
        Level[Level["STRING"] = 1] = "STRING";
    })(Level || (Level = {}));
    var Level$1 = Level;
    ...
    (function () {
        function canSplitRightEndForFree(solutions, lastBridgeIndex) {
            ...
            var result = rightEndLength > 2 || rightEndLength > 1 && !isUnluckyRightEnd(solutions, rightEndIndex);
            return result;
        }
        function findLastBridge(solutions) {
            for (var index = solutions.length; index--;) {
                var bridge = solutions[index].bridge;
                if (bridge)
                    return index;
            }
        }
        ...
        function gatherGroup(solutions, groupBond, groupForceString, bridgeUsed) {
            ...
            if (count > 1) {
                ...
                array = [solution];
                multiPart = false;
                notStr = solution.level < Level$1.STRING;
            }
        ...
        function getNumericJoinCost(level0, level1) {
            var joinCost = level0 > Level$1.UNDEFINED || level1 > Level$1.UNDEFINED ? 2 : 3;
            return joinCost;
        }
        function getSplitCostAt(solutions, index, leftmost, rightmost) {
            ...
            var levelRight;
            var solutionRight;
            var splitCost = (rightmost && levelCenter < Level$1.NUMERIC ?
                3 :
        ...
        function isNumericJoin(level0, level1) {
            var result = level0 < Level$1.OBJECT && level1 < Level$1.OBJECT;
            return result;
        }
        function isUnluckyRightEnd(solutions, firstIndex) {
            var result = solutions[firstIndex].level < Level$1.NUMERIC &&
                solutions[firstIndex + 1].level > Level$1.UNDEFINED;
            return result;
        }
        ...
        function sequence(solutions, offset, count) {
            ...
            if (solution0.level < Level$1.OBJECT && solution1.level < Level$1.OBJECT) {
                if (solution1.level > Level$1.UNDEFINED)
                    array = [solution0, '+[', solution1, ']'];
                else if (solution0.level > Level$1.UNDEFINED)
                    array = ['[', solution0, ']+', solution1];
                else
                    array = [solution0, '+[]+', solution1];
            }
        ...
        function sequenceAsString(solutions, offset, count, emptyReplacement) {
            ...
            if (count) {
                if (count > 1)
                    ...
                    str = solution + (solution.level < Level$1.NUMERIC ? '+[]' : '');
                }
            }
            else
        ...
        var EMPTY_SOLUTION = new Solution('[]', Level$1.OBJECT, false);
        ScrewBuffer =
            function (bond, forceString, groupThreshold, optimizerList) {
                ...
                assignNoEnum(this, {
                    append: function (solution) {
                        ...
                        optimizerList.forEach(function (optimizer) {
                    ...
                    toString: function () {
                        function collectOut(offset, count, maxGroupCount, groupBond) {
                            ...
                            if (count <= groupSize + 1)
                                ...
                                var leftEndCount = _Math_max(halfCount - capacity + capacity % (groupSize - 1), (maxGroupCount / 2 ^ 0) * (groupSize + 1));
                                var leftStr = collectOut(offset, leftEndCount, maxGroupCount);
                                var rightStr = collectOut(offset + leftEndCount, count - leftEndCount, maxGroupCount, true);
                        ...
                        if (solutionCount < 2) {
                            ...
                            var multiPart = forceString && solution.level < Level$1.STRING;
        ...
        optimizeSolutions =
            function (optimizerList, solutions, bond, forceString) {
                ...
                optimizerList.forEach(function (optimizer) {
                    return optimizer.optimizeSolutions(plan, solutions, bond, forceString);
                });
    ...
    (function ()
    {
            ...
            function optimizeSolutions(plan, solutions, bond, forceString)
            {
                for (var index = 0, limit = solutions.length - complexLength; index <= limit; ++index)
                {
                    if (matchComplex(solutions, index))
                    {
                        ...
                        if (!limit)
                        {
                            if (forceString && complexSolution.level < Level$1.STRING)
        ...
        function (encoder, complex, definition)
        {
            ...
            if (complexSolution.level == null)
                complexSolution.level = Level$1.STRING;
    ...
    function maskIncludes(includingMask, includedMask) {
        var part0;
        var part1;
        var included = ((part0 = includedMask[0]) & includingMask[0]) === part0 &&
    ...
    var FEATURE_INFOS = {
        ANY_DOCUMENT: {
            description: 'Existence of the global object document whose string representation starts with ' +
        ...
        ANY_WINDOW: {
            description: 'Existence of the global object self whose string representation starts with "[object " ' +
                'and ends with "Window]".',
            check: checkSelfFeature.bind(function (str) { return /^\[object .*Window]$/.test(str); }),
            includes: ['SELF_OBJ'],
            attributes: { 'web-worker': 'web-worker-restriction' },
        },
        ARRAY_ITERATOR: {
            description: 'The property that the string representation of Array.prototype.entries() starts with ' +
        ...
        BARPROP: {
            description: 'Existence of the global object statusbar having the string representation "[object ' +
                'BarProp]".',
            check: function () {
        ...
        CAPITAL_HTML: {
            description: 'The property that the various string methods returning HTML code such as ' +
        ...
        CONSOLE: {
            description: 'Existence of the global object console having the string representation "[object ' +
                'Console]".\n' +
        ...
        DOCUMENT: {
            description: 'Existence of the global object document having the string representation "[object ' +
                'Document]".',
            check: function () {
        ...
        DOMWINDOW: {
            description: 'Existence of the global object self having the string representation "[object ' +
                'DOMWindow]".',
            check: checkSelfFeature.bind(function (str) { return str === '[object DOMWindow]'; }),
            includes: ['ANY_WINDOW'],
            excludes: ['WINDOW'],
            attributes: { 'web-worker': 'web-worker-restriction' },
        },
        ESC_HTML_ALL: {
            description: 'The property that double quotation mark, less than and greater than characters in the ' +
                'argument of String.prototype.fontcolor are escaped into their respective HTML entities.',
            check: function () { return ~''.fontcolor('"<>').indexOf('&quot;&lt;&gt;'); },
            includes: ['ESC_HTML_QUOT'],
            excludes: ['ESC_HTML_QUOT_ONLY'],
        },
        ESC_HTML_QUOT: {
            description: 'The property that double quotation marks in the argument of String.prototype.fontcolor ' +
                'are escaped as "&quot;".',
            check: function () { return ~''.fontcolor('"').indexOf('&quot;'); },
        },
        ESC_HTML_QUOT_ONLY: {
            description: 'The property that only double quotation marks and no other characters in the argument ' +
        ...
        ESC_REGEXP_LF: {
            description: 'Having regular expressions created with the RegExp constructor use escape sequences ' +
                'starting with a backslash to format line feed characters ("\\n") in their string ' +
                'representation.',
            check: function () { return ("" + RegExp('\n'))[1] === '\\'; },
        },
        ESC_REGEXP_SLASH: {
            description: 'Having regular expressions created with the RegExp constructor use escape sequences ' +
                'starting with a backslash to format slashes ("/") in their string representation.',
            check: function () { return ("" + RegExp('/'))[1] === '\\'; },
        },
        EXTERNAL: {
            description: 'Existence of the global object sidebar having the string representation "[object ' +
                'External]".',
            check: function () { return typeof sidebar === 'object' && "" + sidebar === '[object External]'; },
            attributes: { 'web-worker': 'web-worker-restriction' },
        },
        FF_SRC: {
        ...
        FILL: {
            ...
            check: function () { return Array.prototype.fill; },
        },
        FLAT: {
            description: 'Existence of the native function Array.prototype.flat.',
            check: function () { return Array.prototype.flat; },
        },
        FROM_CODE_POINT: {
            description: 'Existence of the function String.fromCodePoint.',
            check: function () { return String.fromCodePoint; },
        },
        FUNCTION_19_LF: {
            description: 'A string representation of dynamically generated functions where the character at index ' +
                '19 is a line feed ("\\n").',
            check: function () { return ("" + Function())[19] === '\n'; },
        },
        FUNCTION_22_LF: {
            description: 'A string representation of dynamically generated functions where the character at index ' +
                '22 is a line feed ("\\n").',
            check: function () { return ("" + Function())[22] === '\n'; },
        },
        GMT: {
            description: 'Presence of the text "GMT" after the first 25 characters in the string returned by ' +
                'Date().\n' +
                ...
                'Explorer 9 and 10.',
            check: function () { return /^.{25}GMT/.test(Date()); },
        },
        HISTORY: {
            description: 'Existence of the global object history having the string representation "[object ' +
                'History]".',
            check: function () { return typeof history === 'object' && "" + history === '[object History]'; },
            attributes: { 'web-worker': 'web-worker-restriction' },
        },
        HTMLAUDIOELEMENT: {
            description: 'Existence of the global object Audio whose string representation starts with "function ' +
                'HTMLAudioElement".',
            check: function () {
        ...
        INCR_CHAR: {
            description: 'The ability to use unary increment operators with string characters, like in ( ++"some ' +
        ...
        INTL: {
            ...
            check: function () { return typeof Intl === 'object'; },
        },
        LOCALE_INFINITY: {
            description: 'Language sensitive string representation of Infinity as "∞".',
            check: function () { return Infinity.toLocaleString() === '∞'; },
        },
        NAME: {
            description: 'Existence of the name property for functions.',
            check: function () { return 'name' in Function(); },
        },
        NODECONSTRUCTOR: {
            description: 'Existence of the global object Node having the string representation "[object ' +
                'NodeConstructor]".',
            check: function () { return typeof Node !== 'undefined' && "" + Node === '[object NodeConstructor]'; },
            attributes: { 'web-worker': 'web-worker-restriction' },
        },
        NO_FF_SRC: {
            description: 'A string representation of native functions typical for V8 and Edge or for Internet ' +
                'Explorer but not for Firefox and Safari.',
            check: function () { return /^(\n?)function Object\(\) \{\1 +\[native code]\s\}/.test(Object); },
            excludes: ['FF_SRC'],
        },
        NO_IE_SRC: {
            description: 'A string representation of native functions typical for most engines with the notable ' +
            ...
            check: function () { return /^function Object\(\) \{(\n   )? \[native code]\s\}/.test(Object); },
            excludes: ['IE_SRC'],
        },
        NO_OLD_SAFARI_ARRAY_ITERATOR: {
            description: 'The property that the string representation of Array.prototype.entries() evaluates to ' +
        ...
        NO_V8_SRC: {
            description: 'A string representation of native functions typical for Firefox, Internet Explorer and ' +
                'Safari.\n' +
        ...
        SELF_OBJ: {
            description: 'Existence of the global object self whose string representation starts with "[object ".',
            check: checkSelfFeature.bind(function (str) { return /^\[object /.test(str); }),
            attributes: { 'web-worker': 'safari-bug-21820506' },
        },
        STATUS: {
            description: 'Existence of the global string status.',
            check: function () { return typeof status === 'string'; },
            attributes: { 'web-worker': 'web-worker-restriction' },
        },
        UNDEFINED: {
            description: 'The property that Object.prototype.toString.call() evaluates to "[object Undefined]".\n' +
        ...
        UNEVAL: {
            ...
            check: function () { return typeof uneval !== 'undefined'; },
        },
        V8_SRC: {
            description: 'A string representation of native functions typical for the V8 engine, but also found ' +
                'in Edge.\n' +
                'Remarkable traits are the lack of line feed characters at the beginning and at the end ' +
        ...
        WINDOW: {
            description: 'Existence of the global object self having the string representation "[object Window]".',
            check: checkSelfFeature.bind(function (str) { return str === '[object Window]'; }),
            includes: ['ANY_WINDOW'],
            excludes: ['DOMWINDOW'],
            attributes: { 'web-worker': 'web-worker-restriction' },
        },
        DEFAULT: {
            description: 'Minimum feature level, compatible with all supported engines in all environments.',
        },
        BROWSER: {
            description: 'Features available in all browsers.\n' +
    ...
    var FeatureBase = /** @class */ (function () {
    ...
    function areCompatible() {
        var arg0;
        var features = arguments.length === 1 && _Array_isArray(arg0 = arguments[0]) ? arg0 : arguments;
        var compatible;
    ...
    function areEqual() {
        var mask;
        var equal = _Array_prototype_every.call(arguments, function (arg, index) {
            var returnValue;
    ...
    function commonOf() {
        ...
        if (arguments.length) {
            var mask_1;
            _Array_prototype_forEach.call(arguments, function (arg) {
                var otherMask = validMaskFromArrayOrStringOrFeature(arg);
                if (mask_1 != null)
                    mask_1 = maskIntersection(mask_1, otherMask);
                else
                    mask_1 = otherMask;
            });
            returnValue = featureFromMask(mask_1);
        }
        else
            returnValue = null;
        return returnValue;
    ...
    function featureArrayLikeToMask(arrayLike) {
        var mask = maskNew();
        _Array_prototype_forEach.call(arrayLike, function (feature) {
    ...
    function featuresToMask(featureObjs) {
        var mask = maskNew();
        featureObjs.forEach(function (featureObj) {
    ...
    function isMaskCompatible(mask) {
        var compatible = incompatibleMaskList.every(function (incompatibleMask) { return !maskIncludes(mask, incompatibleMask); });
        return compatible;
    }
    /**
    ...
    function maskFromStringOrFeature(arg) {
        ...
        if (arg instanceof Feature)
            ...
            var name_1 = esToString(arg);
            var featureObj = ALL[name_1];
            if (!featureObj)
                throw new _Error("Unknown feature " + _JSON_stringify(name_1));
    ...
    function validMaskFromArguments(args) {
        ...
        _Array_prototype_forEach.call(args, function (arg) {
            var otherMask;
            if (_Array_isArray(arg)) {
    ...
    function wrapCheck(check) {
        var returnValue = function () { return !!check(); };
        return returnValue;
    }
    ...
    var protoSource$1 = {
        get canonicalNames() {
            ...
            var allIncludes = [];
            ELEMENTARY.forEach(function (featureObj) {
                var included = maskIncludes(mask, featureObj.mask);
                if (included) {
                    var name_2 = featureObj.name;
                    featureNameSet[name_2] = null;
                    // An elementary feature can only include elementary features.
                    var includes = includesMap[name_2];
                    _Array_prototype_push.apply(allIncludes, includes);
                }
            });
            allIncludes.forEach(function (name) {
        ...
        includes: function () {
            var mask = this.mask;
            var included = _Array_prototype_every.call(arguments, function (arg) {
                var otherMask = validMaskFromArrayOrStringOrFeature(arg);
        ...
        inspect: inspect,
        name: undefined,
        restrict: function (environment, engineFeatureObjs) {
    ...
    {
        var completeExclusions = function (name) {
            var info = FEATURE_INFOS[name];
            var excludes = info.excludes;
            if (excludes) {
                var featureObj = ALL[name];
                var mask_2 = featureObj.mask;
                excludes.forEach(function (exclude) {
                    var excludeMask = completeFeature_1(exclude);
                    var incompatibleMask = maskUnion(mask_2, excludeMask);
                    incompatibleMaskMap_1[incompatibleMask] = incompatibleMask;
                });
            }
        };
        var completeFeature_1 = function (name) {
            var mask;
            var featureObj = ALL[name];
            if (featureObj)
                (mask = featureObj.mask);
                ...
                if (typeof info === 'string') {
                    ...
                    if (check) {
                        mask = maskWithBit(bitIndex_1++);
                        if (check())
                            autoMask_1 = maskUnion(autoMask_1, mask);
                    ...
                    includes.forEach(function (include) {
                        var includeMask = completeFeature_1(include);
                    ...
                    featureObj =
                        createFeature(name, description, mask, wrappedCheck, engine, info.attributes, elementary);
                    if (elementary)
        ...
        var constructorSource = { ALL: ALL, ELEMENTARY: ELEMENTARY, areCompatible: areCompatible, areEqual: areEqual, commonOf: commonOf };
        assignNoEnum(Feature, constructorSource);
        assignNoEnum(featurePrototype, protoSource$1);
        {
            var inspectKey = void 0;
            try {
                inspectKey = require('util').inspect.custom;
            }
            catch (_a) { }
            if (inspectKey) {
                _Object_defineProperty(featurePrototype, inspectKey, { configurable: true, value: inspect, writable: true });
            }
        }
        var autoMask_1 = maskNew();
        var bitIndex_1 = 0;
        var incompatibleMaskMap_1 = createEmpty();
        var featureNames = _Object_keys(FEATURE_INFOS);
        featureNames.forEach(completeFeature_1);
        featureNames.forEach(completeExclusions);
        incompatibleMaskList =
            _Object_keys(incompatibleMaskMap_1).map(function (key) { return incompatibleMaskMap_1[key]; });
        ELEMENTARY.sort(function (feature1, feature2) {
            return feature1.name < feature2.name ? -1 : 1;
        });
        _Object_freeze(ELEMENTARY);
        var autoFeatureObj = createFeature('AUTO', 'All features available in the current engine.', autoMask_1);
    ...
    function defineList(availableEntries, indexEntries) {
        var effectiveEntries = indexEntries.map(function (indexEntry) {
    ...
    function createBridgeSolution(bridge) {
        var replacement = "[[]]" + bridge + "([[]])";
        var solution = new Solution(replacement, Level$1.OBJECT, false);
    ...
    function createCharDefaultDefinition(charCode, atobOpt, charCodeOpt, escSeqOpt, unescapeOpt) {
        function charDefaultDefinition() {
            var solution = this.createCharDefaultSolution(charCode, atobOpt, charCodeOpt, escSeqOpt, unescapeOpt);
            return solution;
        }
    ...
    defineSimple('false', '![]', Level$1.NUMERIC);
    defineSimple('true', '!![]', Level$1.NUMERIC);
    defineSimple('undefined', '[][[]]', Level$1.UNDEFINED);
    defineSimple('NaN', '+[false]', Level$1.NUMERIC);
    defineSimple('Infinity', JSFUCK_INFINITY, Level$1.NUMERIC);
    (function () {
        var ANY_DOCUMENT = Feature.ANY_DOCUMENT, ANY_WINDOW = Feature.ANY_WINDOW, ARRAY_ITERATOR = Feature.ARRAY_ITERATOR, ARROW = Feature.ARROW, ATOB = Feature.ATOB, BARPROP = Feature.BARPROP, CAPITAL_HTML = Feature.CAPITAL_HTML, CONSOLE = Feature.CONSOLE, DOCUMENT = Feature.DOCUMENT, DOMWINDOW = Feature.DOMWINDOW, ESC_HTML_ALL = Feature.ESC_HTML_ALL, ESC_HTML_QUOT = Feature.ESC_HTML_QUOT, ESC_HTML_QUOT_ONLY = Feature.ESC_HTML_QUOT_ONLY, ESC_REGEXP_LF = Feature.ESC_REGEXP_LF, ESC_REGEXP_SLASH = Feature.ESC_REGEXP_SLASH, EXTERNAL = Feature.EXTERNAL, FF_SRC = Feature.FF_SRC, FILL = Feature.FILL, FLAT = Feature.FLAT, FROM_CODE_POINT = Feature.FROM_CODE_POINT, FUNCTION_19_LF = Feature.FUNCTION_19_LF, FUNCTION_22_LF = Feature.FUNCTION_22_LF, GMT = Feature.GMT, HISTORY = Feature.HISTORY, HTMLAUDIOELEMENT = Feature.HTMLAUDIOELEMENT, HTMLDOCUMENT = Feature.HTMLDOCUMENT, IE_SRC = Feature.IE_SRC, INCR_CHAR = Feature.INCR_CHAR, INTL = Feature.INTL, LOCALE_INFINITY = Feature.LOCALE_INFINITY, NAME = Feature.NAME, NODECONSTRUCTOR = Feature.NODECONSTRUCTOR, NO_FF_SRC = Feature.NO_FF_SRC, NO_IE_SRC = Feature.NO_IE_SRC, NO_OLD_SAFARI_ARRAY_ITERATOR = Feature.NO_OLD_SAFARI_ARRAY_ITERATOR, NO_V8_SRC = Feature.NO_V8_SRC, SELF_OBJ = Feature.SELF_OBJ, STATUS = Feature.STATUS, UNDEFINED = Feature.UNDEFINED, UNEVAL = Feature.UNEVAL, V8_SRC = Feature.V8_SRC, WINDOW = Feature.WINDOW;
                ...
                switch (index) {
                    ...
                    case 30:
                        paddingEntries =
                            [
                                define(10),
                                define({ block: 'RP_6_SO', indexer: 1 + index / 10 + " + FH_SHIFT_1" }, NO_V8_SRC),
                                define(0, V8_SRC),
                                define(5, IE_SRC),
                                define(6, FF_SRC),
                            ];
        ...
        function defineCharDefault(char, opts) {
            ...
            var definition = createCharDefaultDefinition(charCode, atobOpt, charCodeOpt, escSeqOpt, unescapeOpt);
        ...
        var defineFHCharAt = function (expr, index) {
            var entries;
            switch (index) {
                case 3:
                case 13:
        ...
        BASE64_ALPHABET_HI_4 =
            [
                ...
                [
                    ...
                    define('X', ANY_DOCUMENT, ARRAY_ITERATOR, ESC_REGEXP_LF, EXTERNAL, FLAT, FUNCTION_19_LF),
                    define('X', ANY_DOCUMENT, ARRAY_ITERATOR, ESC_REGEXP_LF, EXTERNAL, FILL, FUNCTION_19_LF, NO_FF_SRC),
                    define('V', ANY_DOCUMENT, ARRAY_ITERATOR, ESC_REGEXP_LF, EXTERNAL, FLAT, FUNCTION_19_LF, NO_FF_SRC),
                    define('X', ANY_DOCUMENT, ARRAY_ITERATOR, ESC_REGEXP_LF, EXTERNAL, FLAT, FUNCTION_22_LF, INCR_CHAR),
                    define('V', ANY_DOCUMENT, ARRAY_ITERATOR, ESC_REGEXP_LF, EXTERNAL, FLAT, FUNCTION_22_LF, INCR_CHAR, NO_FF_SRC),
                    define('X', ANY_DOCUMENT, ARRAY_ITERATOR, ESC_REGEXP_LF, FUNCTION_22_LF, HTMLAUDIOELEMENT),
                    define('V', ANY_DOCUMENT, ARRAY_ITERATOR, ESC_REGEXP_LF, FUNCTION_22_LF, HTMLAUDIOELEMENT, V8_SRC),
                    define('X', ANY_DOCUMENT, ARRAY_ITERATOR, ESC_REGEXP_LF, FUNCTION_19_LF, HTMLAUDIOELEMENT),
                    define('V', ANY_DOCUMENT, ARRAY_ITERATOR, ESC_REGEXP_LF, FUNCTION_19_LF, HTMLAUDIOELEMENT, V8_SRC),
                    ...
                    define('V', ANY_DOCUMENT, ARRAY_ITERATOR, ESC_REGEXP_LF, EXTERNAL, FILL, FUNCTION_19_LF, V8_SRC),
                    define('V', ANY_DOCUMENT, ARRAY_ITERATOR, ESC_REGEXP_LF, EXTERNAL, FLAT, FUNCTION_19_LF, NO_IE_SRC),
        ...
        CHARACTERS =
            noProto({
                ...
                'H': [
                    ...
                    define({ expr: '(RP_3_NO + Function("return history")())[11]', optimize: true }, HISTORY),
                ...
                'T': [
                    define({
                        expr: '(Function("try{undefined.false}catch(undefined){return undefined}")() + ' +
                            '[])[0]',
                        optimize: true,
                    }),
                ...
                '∞': [
                    define({ expr: 'Infinity.toLocaleString()', optimize: { complexOpt: true } }, LOCALE_INFINITY),
                    defineCharDefault('∞'),
                ],
            });
        COMPLEX =
            noProto({
                ...
                'f,a,l,s,e': define({ expr: 'F_A_L_S_E', level: Level$1.OBJECT }),
                fromCharCo: define({ expr: '"from3har3o".split(3).join("C")', optimize: { toStringOpt: true } }),
        ...
        CONSTANTS =
            noProto({
                ...
                document: [
                    define({ expr: 'Function("return document")()', optimize: { toStringOpt: true } }, ANY_DOCUMENT),
                ],
                escape: [
        ...
        FROM_CHAR_CODE =
            defineList([define('fromCharCode'), define('fromCodePoint', FROM_CODE_POINT)], [
                define(0),
                define(1, ATOB),
                define(1, BARPROP),
        ...
        FROM_CHAR_CODE_CALLBACK_FORMATTER =
            defineList([
                define(function (fromCharCode, arg) {
                    return "function(undefined){return String." + fromCharCode + "(" + arg + ")}";
                }),
                define(function (fromCharCode, arg) {
                    return "function(undefined){return(isNaN+false).constructor." + fromCharCode + "(" + arg + ")}";
                }),
                define(function (fromCharCode, arg) {
                    return "undefined=>String." + fromCharCode + "(" + arg + ")";
                }, ARROW),
                define(function (fromCharCode, arg) {
                    return "undefined=>(isNaN+false).constructor." + fromCharCode + "(" + arg + ")";
                }, ARROW),
                define(function (fromCharCode, arg) {
                    return "function(undefined){return status.constructor." + fromCharCode + "(" + arg + ")}";
                }, STATUS),
                define(function (fromCharCode, arg) {
                    return "undefined=>status.constructor." + fromCharCode + "(" + arg + ")";
                }, ARROW, STATUS),
            ], [
                define(1),
                define(3),
        ...
        MAPPER_FORMATTER =
            defineList([
                define(function (arg) {
                    return "Function(\"return function(undefined){return this" + arg + "}\")().bind";
                }),
                define(function (arg) {
                    return "Function(\"return falsefalse=>undefined=>falsefalse" + arg + "\")()";
                }, ARROW),
            ], [define(0), define(1)]);
        ...
        for (var digit = 0; digit <= 9; ++digit) {
            ...
            CHARACTERS[digit] = { expr: expr, level: Level$1.NUMERIC };
        }
    })();
    ...
    function expressParse(input) {
        var parseInfo = { data: input, modStack: [], opsStack: [], finalizerStack: [finalizeUnit], unitStack: [] };
    ...
    (function ()
    {
        ...
        var protoSource =
        {
            ...
            function (charCode, atobOpt, charCodeOpt, escSeqOpt, unescapeOpt)
            {
                ...
                var solution = new Solution(replacement, Level$1.STRING, false);
                return solution;
            },
            ...
            function (unit, bondStrength, unitIndices, maxLength, replacers)
            {
                ...
                if (terms = unit.terms)
                {
                    ...
                    var minOutputLevel = Level$1.UNDEFINED;
                    for (var index = 0; index < count; ++index)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                output += '+' + termOutput;
                                minOutputLevel = _Math_max(minOutputLevel, Level$1.NUMERIC);
                            }
                            else
                                output = termOutput;
                        }
